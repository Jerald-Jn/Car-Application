...
public class CarService {
    ...
    public CarService(UserRepo userRepo, InsuranceService insuranceService) {
    ...
    public User addUser(User user, UUID uuid) {
        ...
        for(int i=0;i<check.size();i++){
            if(check.get(i).getCar().getRegistrationNumber().equals(user.getCar().getRegistrationNumber())){
