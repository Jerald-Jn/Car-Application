        ...
        if (token != null && token.startsWith("Bearer ")) {
            ...
            if (userName != null && SecurityContextHolder.getContext().getAuthentication() == null) {
                ...
                if (jwtService.validateToken(token, userDetails)) {
                     ...
                     * userDetails.getAuthorities(): the roles/permissions (e.g. ROLE_USER,
                     * ROLE_ADMIN)
                     */
                    UsernamePasswordAuthenticationToken authToken = new UsernamePasswordAuthenticationToken(userDetails,
                            null, userDetails.getAuthorities());
                    // Adds additional details to the authentication object, those are IP address,
                    // Session ID, User agent
                    authToken.setDetails(new WebAuthenticationDetailsSource().buildDetails(request));
                    // Sets the built authToken into the Spring Security context, marking the user
                    // as authenticated for this request thread.
